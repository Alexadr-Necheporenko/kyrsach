...
async def process_settings(message: Message):
    ...
    await message.answer('Настройка категорій:', reply_markup=markup)
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.answer('Усі добавлені товари до цієї категорії.')
...
async def process_cancel(message: Message, state: FSMContext):
    ...
    await message.answer('Ок, відмінено!', reply_markup=ReplyKeyboardRemove())
    await state.finish()
...
async def process_image_url(message: Message, state: FSMContext):
    ...
    if message.text == back_message:
        ...
        async with state.proxy() as data:
            ...
            await message.answer(f"Змінити опис с <b>{data['body']}</b>?", reply_markup=back_markup())

    else:

        await message.answer('Вам потрібно надати фото товару.')
...
async def process_price_invalid(message: Message, state: FSMContext):
    ...
    if message.text == back_message:
        ...
        async with state.proxy() as data:
            ...
            await message.answer("Інше зображення?", reply_markup=back_markup())

    else:

        await message.answer('Вкажіть ціну у виді числа!')
...
async def process_confirm_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer(f"Змінити ціну с <b>{data['price']}</b>?", reply_markup=back_markup())
