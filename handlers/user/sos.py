...
async def cmd_sos(message: Message):
    ...
    await message.answer("В чому полягає проблема? Опишіть як можно детальніше та адміністратор обов'язково вам відповість.", reply_markup=ReplyKeyboardRemove())
...
async def process_submit(message: Message, state: FSMContext):
    ...
    if db.fetchone('SELECT * FROM questions WHERE cid=?', (cid,)) == None:
        ...
        await message.answer('СПАМ.Перевищений ліміт запитів.', reply_markup=ReplyKeyboardRemove())
